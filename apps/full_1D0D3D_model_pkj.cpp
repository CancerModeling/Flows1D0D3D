...
int main(int argc, char *argv[]) {
  ...
  options.add_options()                                                                                                         //
    ...
    ("tau-coup", "time step size for updating the coupling", cxxopts::value<double>()->default_value("1e-2"))                   //
    ("t-end", "Simulation period for simulation", cxxopts::value<double>()->default_value("1.e1"))                             //
...

  {
    ...
    if (filename.empty()) {
      input.d_T = t_end;
      input.d_dt = coupling_interval * tau; // 3D time step as multiple of 1D time step
