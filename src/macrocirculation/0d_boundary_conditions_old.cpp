...
void convert_rcr_to_partitioned_tree_bcs(const std::shared_ptr<GraphStorage> &graph)
{
  for (auto &v_id : graph->get_vertex_ids()) {
    ...
    if (vertex.is_windkessel_outflow())
    {
      ...
      // arterioles
      list_C.push_back(C*0.01); // 8
      list_C.push_back(C*0.02); // 9
      list_C.push_back(C*0.03); // 10
      list_C.push_back(C*0.04); // 10
      list_C.push_back(C*0.05); // 11
      list_C.push_back(C*0.06); // 12
      list_C.push_back(C*0.07); // 0.48
      list_C.push_back(C*0.07); //1.98
      // capillaries
      list_C.push_back(C*0.15); //15
      // venules
      list_C.push_back(C*0.20); //20
      // small veins
      list_C.push_back(C*0.30); // 40
