...
CoupledExplicitImplicit1DSolver::CoupledExplicitImplicit1DSolver(
  ...
  d_coupling = std::make_shared<NonlinearLinearCoupling>(MPI_COMM_WORLD, d_graph_nl, d_graph_li, d_explicit_solver, d_implicit_solver);

  d_explicit_solver->use_explicit_euler_method();
}

void CoupledExplicitImplicit1DSolver::add_coupled_vertices(const std::string &name_vertex_nl, const std::string &name_vertex_li) {
  if (d_is_setup)
    throw std::runtime_error("coupling vertices is not posssible after setup");
  d_coupling->add_coupled_vertices(name_vertex_nl, name_vertex_li);
}
...
void CoupledExplicitImplicit1DSolver::solve(double tau, double t) {
  ...
  d_coupling->update_linear_solver();
  d_implicit_solver->solve(d_tau, t + d_tau);
  d_coupling->update_nonlinear_solver();
}
