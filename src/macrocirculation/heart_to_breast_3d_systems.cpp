...
void CapillaryNutrient::assemble_1d() {
  ...
  auto &out_elems = d_model_p->d_perf_elems_3D;
  double r_nut = 1. - input.d_rnut_art_cap;
  auto &p_cap = d_model_p->d_p_cap;
  ...
  for (size_t I = 0; I < pts.size(); I++) {
    ...
    for (const auto &elem_id : out_elems[I]) {
      ...
      for (unsigned int qp = 0; qp < d_qrule.n_points(); qp++) {
        double p_qp = 0.;
        for (unsigned int l = 0; l < d_phi.size(); l++) {
          p_qp += d_phi[l][qp] * p_cap.get_current_sol(l);
        }

        if (pI - p_qp > 0.) {
          // to rhs
          for (unsigned int i = 0; i < d_phi.size(); i++) {
            d_Fe(i) += d_JxW[qp] * r_nut * input.d_Lp_art_cap
                       * (pI - p_qp) * (*out_fn_I)(d_qpoints[qp]) * nutI *d_phi[i][qp];
          }
        } else {
          // to lhs
          for (unsigned int i = 0; i < d_phi.size(); i++) {
            for (unsigned int j = 0; j < d_phi.size(); j++) {
              d_Ke(i, j) += -d_JxW[qp] * r_nut * input.d_Lp_art_cap
                * (pI - p_qp) * (*out_fn_I)(d_qpoints[qp]) * d_phi[j][qp] *d_phi[i][qp];
            }
          }
        }
