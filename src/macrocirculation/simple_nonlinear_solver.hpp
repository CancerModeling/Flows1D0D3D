...
#ifndef SIMPLE_NONLINEAR_SOLVER_H
#define SIMPLE_NONLINEAR_SOLVER_H
...
class SimpleNonlinearSolver {
public:
  SimpleNonlinearSolver(MPI_Comm comm, const std::string & filepath, const std::string& folder, const std::string& name, double tau);

  SimpleNonlinearSolver(const std::string & filepath, const std::string& folder, const std::string& name, double tau = 1e-5);
...
private:
  ...
  std::shared_ptr<ExplicitNonlinearFlowSolver> solver;
