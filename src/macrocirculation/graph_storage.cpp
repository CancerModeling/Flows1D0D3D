...
void Vertex::set_to_inflow(std::function<double(double)> value) {
  ...
  if (is_windkessel_outflow())
    std::cout << "warning: changing windkessel to inflow after distributing dofs might result in lost dofs " << std::endl;
  if (is_vessel_tree_outflow() && get_vessel_tree_data().resistances.size() > 0)
    std::cout << "warning: changing vessel_tree_outflow to inflow after distributing dofs might result in loss of dofs " << std::endl;
...
void Vertex::set_to_free_outflow() {
  ...
  if (is_windkessel_outflow())
    std::cout << "warning: changing windkessel to free outflow after distributing dofs might result in lost dofs " << std::endl;
  if (is_vessel_tree_outflow() && get_vessel_tree_data().resistances.size() > 0)
    std::cout << "warning: changing vessel_tree_outflow to free outflow after distributing dofs might result in loss of dofs " << std::endl;
...
void Vertex::set_to_windkessel_outflow(double r, double c) {
  ...
  if (is_vessel_tree_outflow() && get_vessel_tree_data().resistances.size() > 0)
    std::cout << "warning: changing vessel_tree_outflow to windkessel after distributing dofs might result in loss of dofs " << std::endl;
...
void Vertex::set_to_linear_characteristic_inflow(double C, double L, bool points_towards_vertex, double p, double q) {
  ...
  if (is_windkessel_outflow())
    std::cout << "warning: changing windkessel to characteristic inflows after distributing dofs might result in lost dofs " << std::endl;
  if (is_vessel_tree_outflow() && get_vessel_tree_data().resistances.size() > 0)
    std::cout << "warning: changing vessel_tree_outflow to characteristic inflows after distributing dofs might result in loss of dofs " << std::endl;
