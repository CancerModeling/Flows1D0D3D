 ...
 */
class BaseAssembly : public lm::System::Assembly {
...
public:
   ...
   * @param sys System
   */
  BaseAssembly(const std::string system_name, lm::MeshBase &mesh,
               lm::TransientLinearImplicitSystem &sys, const unsigned int &num_vars,
      ...
      : d_sys_name(system_name), d_mesh(mesh), d_sys(sys), d_num_vars(num_vars),
        ...
        d_fe(lm::FEBase::build(d_mesh.mesh_dimension(), d_fe_type)),
        ...
        d_fe_face(lm::FEBase::build(d_mesh.mesh_dimension(), d_fe_type)),
    ...
    if (d_num_vars > 1) {
      ...
      d_Fe_var = std::vector<lm::DenseSubVector<lm::Number>>(d_num_vars, d_Fe);
      d_Ke_var = std::vector<std::vector<lm::DenseSubMatrix<lm::Number>>>(
        d_num_vars, std::vector<lm::DenseSubMatrix<lm::Number>>(d_num_vars, d_Ke));
    }
  }
   ...
   */
  void init_fe(const lm::Elem *elem) {
   ...
   * @param elem Pointer to the element
   */
  void init_face_fe(const lm::Elem *elem, unsigned short side) {
   ...
   * @param elem Pointer to the element
   */
  void init_dof(const lm::Elem *elem) {
   ...
   * @param elem Pointer to the element
   */
  void init_var_dof(const lm::Elem *elem) {
   ...
   */
  void init_dof(const lm::Elem *elem, std::vector<unsigned int> &dof_indices_sys) {
   ...
   */
  void
  init_var_dof(const lm::Elem *elem, std::vector<unsigned int> &dof_indices_sys,
   ...
   */
  void init_localized_sol(const lm::Parallel::Communicator &comm) {
    d_localized_sol = lm::NumericVector<lm::Number>::build(comm);
    d_localized_sol->init(d_sys.solution->size(), false, lm::SERIAL);
    //if (comm.rank() == 0)
...
public:
  ...
  lm::TransientLinearImplicitSystem &d_sys;

  /*! @brief Mesh */
  const lm::MeshBase &d_mesh;
  ...
  /*! @brief FE object */
  lm::UniquePtr<lm::FEBase> d_fe;

  /*! @brief Reference to quadrature data */
  const std::vector<lm::Point> &d_qpoints;
  const std::vector<lm::Real> &d_JxW;
  const std::vector<std::vector<lm::Real>> &d_phi;
  const std::vector<std::vector<lm::RealGradient>> &d_dphi;

  /*! @brief Quadrature points information for integral over face */
  lm::QGauss d_qrule_face;

  /*! @brief FE object */
  lm::UniquePtr<lm::FEBase> d_fe_face;

  /*! @brief Reference to quadrature data */
  const std::vector<lm::Point> &d_qpoints_face;
  const std::vector<lm::Real> &d_JxW_face;
  const std::vector<std::vector<lm::Real>> &d_phi_face;
  const std::vector<std::vector<lm::RealGradient>> &d_dphi_face;
  const std::vector<lm::Point> &d_qface_normals;

  /*! @brief Local matrix and vector */
  lm::DenseMatrix<lm::Number> d_Ke;
  lm::DenseVector<lm::Number> d_Fe;

  /*! @brief Local matrix and vector for multi-variable systems */
  std::vector<std::vector<lm::DenseSubMatrix<lm::Number>>> d_Ke_var;
  std::vector<lm::DenseSubVector<lm::Number>> d_Fe_var;

  /*! @brief Localized solution vector */
  std::unique_ptr<lm::NumericVector<lm::Number>> d_localized_sol;
  std::vector<lm::Number> d_localized_sol_std;
