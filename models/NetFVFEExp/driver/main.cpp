...
int main(int argc, char *argv[]) {
  ...
  std::string model_name = input_file("model_name", "NetFVFEExp");

  if (model_name == "NetFVFEExp") {
    ...
    // Run the model
    netfvfeexp::model_setup_run(argc, argv, filename, &init.comm());

  } else {
    std::cout << "Model name = " << model_name
              << " is incorrect. It should be NetFVFEExp to run network-tumor "
