...
Number netfvfeexp::initial_condition_hyp(const Point &p, const Parameters &es,
...
// Assembly class
void netfvfeexp::HypAssembly::assemble() {

  assemble_1();
}

void netfvfeexp::HypAssembly::assemble_1() {
  ...
  for (const auto &elem : d_mesh.active_local_element_ptr_range()) {
    ...
    for (unsigned int qp = 0; qp < d_qrule.n_points(); qp++) {
      ...
      mobility = 1.e-8 + deck.d_bar_M_H * pow(hyp_proj, 2) * pow(1. - tum_proj, 2);
