  ...
  for (const auto &elem : d_mesh.active_local_element_ptr_range()) {
    ...
    for (unsigned int qp = 0; qp < d_qrule.n_points(); qp++) {
      ...
      if (deck.d_assembly_method == 1) {
        ...
        compute_rhs = d_JxW[qp] * (taf_old + dt * deck.d_lambda_TAF * hyp_cur * util::heaviside(hyp_proj - deck.d_sigma_HTAF));
        compute_mat = d_JxW[qp] * (1. + dt * deck.d_lambda_TAF * hyp_cur + dt * deck.d_lambda_TAF_deg);
