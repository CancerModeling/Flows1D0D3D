...
void util::unetfc::Network::create_initial_network() {
...
void util::unetfc::Network::solve3D1DFlowProblem(BaseAssembly &pres_sys,
...
void util::unetfc::Network::solve3D1DNutrientProblem(BaseAssembly &nut_sys,
                                                     BaseAssembly &tum_sys) {
  ...
  while (pointer) {
    ...
    // Debug
    //  Disable below as it was not present in semi-coupled
...
void util::unetfc::Network::solveVGMforPressure(BaseAssembly &pres_sys) {
...
void util::unetfc::Network::solveVGMforNutrient(BaseAssembly &pres_sys,
...
void util::unetfc::Network::assemble3D1DSystemForPressure(
      ...
      for (int k = 0; k < N_3D; k++) { // z-loop
        ...
        for (int face = 0; face < 6; face++) {
          ...
          if (isInnerFace) {
            ...
            // std::cout << "A_flow_3D1D( index, index_neighbor ): " <<
            // A_flow_3D1D( index, index_neighbor ) << std::endl;
            ...
            if (scenario == "test_single_vessel") {
              ...
              b_flow_3D1D[index] =
                  b_flow_3D1D[index] +
                  (2.0 * K_3D * h_3D * h_3D * dirichlet_value) / h_3D;
            }
          }

          // std::cout << "A_flow_3D1D( index, index ): " << A_flow_3D1D( index,
          // index ) << std::endl;
        }
      }
    }
...
void util::unetfc::Network::assemble3D1DSystemForNutrients(
  ...
  while (pointer) {
    ...
    if (numberOfNeighbors == 1) {
      ...
      // init
      A_nut_3D1D(N_tot_3D + indexOfNode, N_tot_3D + indexOfNode) = 0.;
...
void util::unetfc::Network::assembleVGMSystemForPressure(
...
void util::unetfc::Network::assembleVGMSystemForNutrient(
    ...
    if (numberOfNeighbors == 1) {
      ...
      for (int i = 0; i < numberOfNeighbors; i++) {
        ...
        // coupling between 3d and 1d nutrient
        {
          ...
          Ac_VGM(indexOfNode, indexOfNode) += factor_c * dt * coupling_theta *
                                              pointer->L_s[i] *
                                              J_b_data.half_cyl_surf;
          ...
          for (unsigned int e = 0; e < J_b_data.elem_id.size(); e++) {
            ...
            // explicit
            double source = dt * pointer->L_s[i] * J_b_data.half_cyl_surf *
                            e_w * ((1. - coupling_theta) * c_couple - c_t_k);
...
double util::unetfc::Network::getDirichletValue(std::vector<double> center_face,
...
double util::unetfc::Network::getK1D(double s, double L_p, double radius) {
