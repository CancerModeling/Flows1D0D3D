...
void util::unet::Network::assemble3D1DSystemForNutrientsOld(
    BaseAssembly &nut_sys, BaseAssembly &tum_sys) {
  ...
  while (pointer) {
    ...
    if (numberOfNeighbors == 1) {
      ...
      radius = pointer->radii[0];

      indexNeighbor = pointer->neighbors[0]->index;

      coord_neighbor = pointer->neighbors[0]->coord;

      length = util::dist_between_points(coord, coord_neighbor);

      p_neighbor = pointer->neighbors[0]->p_v;
      ...
      if (v_interface > 0.0 && pointer->typeOfVGNode == TypeOfNode::DirichletNode) {
        ...
        A_flow_3D1D(N_tot_3D+indexOfNode, N_tot_3D+indexOfNode) = A_flow_3D1D(N_tot_3D+indexOfNode, N_tot_3D+indexOfNode) + dt * v_interface + 2.0 * dt * D_v / length;
        ...
        A_flow_3D1D(N_tot_3D+indexOfNode, N_tot_3D+indexNeighbor) = dt * v_interface - dt * D_v / length;

        A_flow_3D1D(N_tot_3D+indexOfNode, N_tot_3D+indexOfNode) = A_flow_3D1D(N_tot_3D+indexOfNode, N_tot_3D+indexOfNode) - dt * v_interface + dt * D_v / length;
      ...
      for (int i = 0; i < numberOfNeighbors; i++) {
        ...
        A_flow_3D1D(N_tot_3D + indexOfNode, N_tot_3D + indexNeighbor) = 0.0;
        ...
        v_interface = -(radius * radius * M_PI) / (8.0 * length * mu) * (p_neighbor - p_v);

        // mass matrix
        A_flow_3D1D(N_tot_3D + indexOfNode, N_tot_3D + indexOfNode) += length * 0.5;
