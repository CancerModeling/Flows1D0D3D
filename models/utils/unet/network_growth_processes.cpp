...
void util::unet::Network::updateNetwork(BaseAssembly &taf_sys, BaseAssembly &grad_taf_sys) {
  ...
  std::cout << " " << std::endl;
  std::cout << "Number of nodes after linking terminal vessels to the network: "
...
void util::unet::Network::markApicalGrowth() {
  ...
  while (pointer) {
    ...
    if (pointer->typeOfVGNode == TypeOfNode::DirichletNode &&
      ...
      if (0.01 < coord[0] && coord[0] < L_x - 0.01 && 0.01 < coord[1] &&
          coord[1] < L_x - 0.01 && 0.01 < coord[2] && coord[2] < L_x - 0.01) {
    ...
    if (pointer->apicalGrowth) {
      ...
      if (TAF_point < TAF_max) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          direction[i] = (0.5 * (max_vec[i] + max_vec_2[i]) - coord[i]) +
                         (0.5 * normal_plane[i]);
        }
        ...
        for (int i = 0; i < 3; i++) {
          ...
          direction[i] = (0.75 * near_node[i] + 0.25 * max_vec[i] - coord[i]) +
                         (0.5 * normal_plane[i]);
        }
      }
      ...
      bool isIntersecting =
          testIntersection(coord, new_point_1, new_point_link, radius_p);
      ...
      if (!bifurcate && length_d > 0.0) {
        ...
        if (!isIntersecting) {
          ...
          createALinkingNode(new_point_1, radius_p, pointer);
        }

      } else if (bifurcate && length_d > 0.0) {
...
void util::unet::Network::removeRedundantTerminalVessels() {
  ...
  while (pointer) {
    ...
    if (numberOfNeighbors == 1) {
      ...
      // if node is near the boundary, we do not process Omega = (0,L)^3
      if (0.02 < coord[0] && coord[0] < L_x - 0.02 && 0.02 < coord[1] &&
          coord[1] < L_x - 0.02 && 0.02 < coord[2] && coord[2] < L_x - 0.02) {
  ...
  while (pointer) {
    ...
    if (numberOfNeighbors == 1 && pointer->notUpdated > 2) {
      ...
      for (int i = 0; i < numberOfNeighbors_neighbor; i++) {
        ...
        std::cout << "pointer->neighbors[ 0 ]->neighbors[ i ]->index: "
                  << pointer->neighbors[0]->neighbors[i]->index << std::endl;
        ...
        sproutingProbability =
            ...
            0.5 * std::erf((std::log(log_dist) - input.d_log_normal_mean) /
                           std::sqrt(2.0 * input.d_log_normal_std_dev *
                                     input.d_log_normal_std_dev));
        /*
                            std::cout << "TAF-TAF_th: " << TAF-TAF_th <<
           std::endl; std::cout << "length: " << length << std::endl; std::cout
           << "sproutingProbability: " << sproutingProbability << std::endl;
        */
        double global_max_TAF = gmm::vect_norminf(phi_TAF);

        if (sproutingProbability > 0.9 && TAF > TAF_th && length > 0.025) {
  ...
  while (pointer) {
    ...
    for (int i = 0; i < numberOfEdges; i++) {
      ...
      if (pointer->sprouting_edge[i] == true &&
        ...
        std::vector<double> max_center =
            getCenterFromIndex(max_index, N_3D, h_3D);
        std::vector<double> max_center_2 =
            getCenterFromIndex(max_index_2, N_3D, h_3D);
        ...
        if (angle * 180.0 / M_PI > 20.0 && angle * 180.0 / M_PI < 90.0 &&
            norm_dir_new_vessel > 0.0 && norm_dir_vessel > 0.025 &&
            !isColliding) {
...
bool util::unet::Network::testIntersection(
    std::vector<double> point_1, std::vector<double> point_2,
    std::vector<double> &new_point_link, double radius) {
    ...
    for (int i = 0; i < numberOfNeighbors; i++) {
      ...
      if (pointer->edge_touched[i] == false && !isIntersecting) {
        ...
        if (util::dist_between_points(point_1, point_p1) < 1.0e-11 ||
          ...
          std::vector<std::vector<double>> points_p;
          std::vector<std::vector<double>> points_test_edge;
          ...
          for (int j = 1; j < 10; j++) {
            ...
            for (int k = 0; k < 3; k++) {
              ...
              new_point_p[k] =
                  point_p1[k] + (length_p * (double)j / 10.0 * dir_p[k]);
              new_point_test[k] =
                  point_1[k] + (length_test * (double)j / 10.0 * dir_test[k]);
            }

            if (util::dist_between_points(new_point_p, new_point_test) <
                radius + radius_p) {

              new_point_link = new_point_p;
