...
void netfc::boundary_condition_nut(EquationSystems &es) {
...
void netfc::NutAssembly::assemble() {
  ...
  for (const auto &elem : mesh.active_local_element_ptr_range()) {
    ...
    Number nut_cur = nut.current_solution(dof_indices_nut[0]);
    {
      for (unsigned int qp = 0; qp < qrule.n_points(); qp++) {
        ...
        // add source
        Fe(0) += factor_nut * JxW[qp] *
                 (dt * deck.d_lambda_A * (tum_cur - nec_cur) +
...
void netfc::NutAssembly::assemble_2() {
    ...
    while (pointer) {
      ...
      for (int i = 0; i < numberOfNeighbors; i++) {
        ...
        for (unsigned int e = 0; e < J_b_data.elem_id.size(); e++) {
          ...
          Fe(0) += dt * factor_nut * pointer->L_s[i] * J_b_data.half_cyl_surf *
                   e_w *
                   (c_v_k - (1. - deck.d_coupling_method_theta) * c_t_k_proj);
...
void netfc::NutAssembly::assemble_3() {
