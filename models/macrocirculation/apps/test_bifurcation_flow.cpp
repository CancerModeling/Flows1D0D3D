...
void test1() {
  ...
  std::cout << "Q_a_up = " << Q_up[0] << ", A_a_up = " << A_up[0] << std::endl;
  std::cout << "Q_b_up = " << Q_up[1] << ", A_b_up = " << A_up[1] << std::endl;
  std::cout << "Q_c_up = " << Q_up[2] << ", A_c_up = " << A_up[2] << std::endl;

  std::cout << "diff Q = " << Q_up[0] - Q_up[1] - Q_up[2] << std::endl;
  std::cout << "diff p1 = " << calculate_p_from_QA(Q_up[0], A_up[0], param_a) - calculate_p_from_QA(Q_up[1], A_up[1], param_b) << std::endl;
  std::cout << "diff p2 = " << calculate_p_from_QA(Q_up[0], A_up[0], param_a) - calculate_p_from_QA(Q_up[2], A_up[2], param_c) << std::endl;
}
...
void test2() {
...

  {
    ...
    std::cout << "diff Q = " << Q_up[0] - Q_up[1] - Q_up[1] << std::endl;
    std::cout << "diff p1 = " << calculate_p_from_QA(Q_up[0], A_up[0], param_a) - calculate_p_from_QA(Q_up[1], A_up[1], param_b) << std::endl;
    std::cout << "diff p2 = " << calculate_p_from_QA(Q_up[0], A_up[0], param_a) - calculate_p_from_QA(Q_up[2], A_up[2], param_c) << std::endl;
  }
...
void test3() {
  ...
  std::cout << "Q_b_up = " << Q_up[1] << ", A_b_up = " << A_up[1] << std::endl;
  std::cout << "Q_c_up = " << Q_up[2] << ", A_c_up = " << A_up[2] << std::endl;
  std::cout << "Q_d_up = " << Q_up[3] << ", A_d_up = " << A_up[3] << std::endl;

  std::cout << "diff Q = " << Q_up[0] - Q_up[1] - Q_up[2] - Q_up[3] << std::endl;
  std::cout << "diff p1 = " << calculate_p_from_QA(Q_up[0], A_up[0], param_a) - calculate_p_from_QA(Q_up[1], A_up[1], param_b) << std::endl;
  std::cout << "diff p2 = " << calculate_p_from_QA(Q_up[0], A_up[0], param_a) - calculate_p_from_QA(Q_up[2], A_up[2], param_c) << std::endl;
  std::cout << "diff p3 = " << calculate_p_from_QA(Q_up[0], A_up[0], param_a) - calculate_p_from_QA(Q_up[3], A_up[3], param_d) << std::endl;
}
