...
class ConnectionPair:
    def __init__(self, name0, name1, length, number_micro_edges, radius, elastic_modulus, wall_thickness):
        self.name0 = name0
        self.name1 = name1
        self.length = length
...
to_connect = [
    ConnectionPair('cw_con_1', 'bg_132', 10., 240, radius=0.24, elastic_modulus=1300000.0, wall_thickness=0.005),
    ConnectionPair('cw_con_2', 'bg_135', 10., 240, radius=0.24, elastic_modulus=1300000.0, wall_thickness=0.005)
]
...
for cpair in to_connect:
    ...
    new_microedge = {
        'id': max_vessel_id + 1,
        'name': 'con_vertex({})_vertex({})'.format(cpair.name0, cpair.name1),
        'vessel_length': cpair.length,
        'radius': cpair.radius,
        'wall_thickness': cpair.wall_thickness,
        'elastic_modulus': cpair.elastic_modulus,
        'gamma': 2,
        'number_edges': cpair.number_micro_edges,
        'left_vertex_id': v0['id'],
        'right_vertex_id': v1['id'],
    }
    geo['vessels'] += [new_microedge]
    max_vessel_id += 1
