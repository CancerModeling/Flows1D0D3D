...
def gen_tumor_ic_file(L, filename):
    ...
    inpf.write("{}, {}, {}, {}, {}, {}, {}, {}, {}, {}\n".format(1, 0.75*L, 0.75*L, 0.5*L, 0.2*L, 0.0, 0.0, 0.25*L, 0.0, 0.0, 0.0))
...
def input():
    ...
    add(param_index, param_val, 'model_name', 'AvaFV')
    ...
    break_points.append(len(param_val))
    break_msg.append('\n# output')
    dt_output = np.floor(final_t / delta_t) / 100
    ...
    add(param_index, param_val, 'linear_solver_tol', 1.0e-6)
    add(param_index, param_val, 'nonlinear_solver_max_iter', 50)
    add(param_index, param_val, 'nonlinear_solver_tol', 1.0e-6)

    # nutrient
    break_points.append(len(param_val))
    break_msg.append('\n# nutrient')
    add(param_index, param_val, 'lambda_P', 2.5)
    add(param_index, param_val, 'lambda_A', 0.0)
    add(param_index, param_val, 'lambda_Ph', 0.5)
    add(param_index, param_val, 'D_sigma', 100.)
    add(param_index, param_val, 'delta_sigma', 1.)
    add(param_index, param_val, 'chi_c', 0.04)

    # tumor
    break_points.append(len(param_val))
    break_msg.append('\n# tumor')
    add(param_index, param_val, 'bar_M_P', 200.)
    add(param_index, param_val, 'bar_E_phi_T', 0.045)
    add(param_index, param_val, 'epsilon_T', 1.6e-2)

    # hypoxic
    break_points.append(len(param_val))
    break_msg.append('\n# hypoxic')
    add(param_index, param_val, 'bar_M_H', 20.)
    add(param_index, param_val, 'lambda_PH', 0.5)
    add(param_index, param_val, 'lambda_HP', 0.5)
    add(param_index, param_val, 'lambda_HN', 0.5)
    add(param_index, param_val, 'sigma_PH', 0.4)
    add(param_index, param_val, 'sigma_HP', 0.5)
    add(param_index, param_val, 'sigma_HN', 0.3)
    ...
    break_points.append(len(param_val))
    break_msg.append('\n# TAF')
    add(param_index, param_val, 'D_TAF', 10.)
    ...
    add(param_index, param_val, 'vessel_D_sigma', 5.0)
    ...
    add(param_index, param_val, 'tissue_flow_K', 1.0e-4)
    add(param_index, param_val, 'tissue_flow_density', 1.)
    L_p = 1.0e-4
    add(param_index, param_val, 'tissue_flow_L_p', L_p)
    add(param_index, param_val, 'tissue_nut_L_s', 1.e-1)
    ...
    break_points.append(len(param_val))
    break_msg.append('\n# nutrient ic')
    add(param_index, param_val, 'ic_nutrient_value', 0.4)
