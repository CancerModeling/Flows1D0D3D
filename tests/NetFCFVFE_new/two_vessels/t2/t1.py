...
def gen_init_network_file(L, R, pressures, filename):
    ...
    inpf.write("{} {} {} {}\n".format(L - 3.*R, L - 3.*R, 0.0, pressures[0]))
    inpf.write("{} {} {} {}\n".format(L - 3.*R, L - 3.*R, L, pressures[1]))
    inpf.write("{} {} {} {}\n".format(3.*R, 3.*R, 0.0, pressures[2]))
    inpf.write("{} {} {} {}\n".format(3.*R, 3.*R, L, pressures[3]))
...
def network_input(L, pp_tag, vessel_radius, pressures, network_file, file_dir, num_refinement, param_index, param_val):
    ...
    add(param_index, param_val, 'network_update_interval', 2)

    # control parameters for growth algorithm
    add(param_index, param_val, 'vessel_lambda_g', 1.0)
    add(param_index, param_val, 'vessel_R_factor', 1.0)
    add(param_index, param_val, 'log_normal_mean', 1.0)
    add(param_index, param_val, 'log_normal_std_dev', 0.2)
    add(param_index, param_val, 'network_radius_exponent_gamma', 3.0)
    ...
    add(param_index, param_val, 'network_update_taf_threshold', 1.0e-4)
    ...
    add(param_index, param_val, 'network_min_radius', 9.0e-3)
    add(param_index, param_val, 'network_bifurcate_probability', 0.92)
    add(param_index, param_val, 'network_sprouting_prob', 0.93)
...
def input(L, file_dir, pp_tag, model_name, test_name, output_debug_info, advection_active, decouple_nutrients, num_elems, final_time, delta_t, dt_output, chi_c, D_sigma, nutrient_ic_val, tissue_flow_coef, vessel_D_sigma, L_p, L_s, vessel_pressures, vessel_radius, tumor_ic_type, tumor_ic_center, tumor_ic_radius, vessel_num_refinement, lambda_P, mobility_P):
    ...
    break_points.append(len(param_val))
    break_msg.append('\n# TAF')
    add(param_index, param_val, 'D_TAF', 5.0e-1)
    ...
    add(param_index, param_val, 'scenario', 'two_vessels')
...
chi_c = 0.0
D_sigma = 3.0
nutrient_ic_val = 0.01
if test_name != 'test_nut':
    nutrient_ic_val = 0.5
tissue_flow_coef = 1.e-9
vessel_D_sigma = 7.0e-3
L_p = 1.0e-7
L_s = 4.5
vessel_pressures = [100., 800., 3000., 850.]
vessel_radius = 0.0625
tumor_ic_type = 1
tumor_ic_center = [0.55 * L, 0.45 * L, 0.5 * L]
tumor_ic_radius = [0.15 * L, 0.15 * L, 0.15 * L]
vessel_num_refinement = 3
lambda_P = 5.
mobility_P = 50.
vessel_filename = 'artery_vein.dgf'
